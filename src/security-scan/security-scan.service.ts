...
export class SecurityScanService {
  ...
  async scanRepository(
    ...
    if (!forceScan && lastScanRecord) {
      ...
      this.logger.log(
        `Last scan commit hash: ${lastScanRecord.lastCommitHash}`,
      );

      const changeResult = await this.scmManager.hasChangesSince(
        repoUrl,
        lastScanRecord.lastCommitHash,
      );
      this.logger.log(`Change detection result:`, changeResult);

      if (changeResult.result && !changeResult.result.hasChanges) {
        this.logger.log(
          `No changes detected for ${repoUrl}, returning no-change finding instead of performing scan`,
        );
        const metadataResult =
          await this.scmManager.fetchRepositoryMetadata(repoUrl);
        const metadata =
          metadataResult.metadata || this.getDefaultMetadata(repoUrl);
      ...
      this.logger.log(
        `No previous scan record found for ${repoUrl} or force scan requested`,
      );
    }
    ...
    try {
      ...
      const cloneResult = await this.scmManager.cloneRepository(
        repoUrl,
        repoPath,
      );
      ...
      this.logger.log(
        `Successfully cloned repository using provider: ${cloneResult.provider}`,
      );
      ...
      // 4. Run all scanners
      const allFindings: { [scannerName: string]: any[] } = {};
      const scannerInfos: Array<{
        name: string;
        version: string;
        findings: any[];
      }> = [];
        ...
        try {
          ...
          const findingsWithContext = await Promise.all(
            findings.map(async (finding) => {
              try {
                this.logger.log(
                  `Extracting context for file: ${finding.filePath}, line: ${finding.line}`,
                );
                ...
                this.logger.log(
                  `Context extraction result: ${codeContext ? 'SUCCESS' : 'NULL'} for ${finding.filePath}`,
                );
                return {
                  ...finding,
                ...
                this.logger.warn(
                  `Failed to extract code context for ${finding.filePath}:${finding.line}: ${error.message}`,
                );
                return {
                  ...finding,
                  scanner: scannerInfo.name,
          ...
          this.logger.log(
            `${scannerInfo.name} found ${findings.length} issues`,
          );
        } catch (error) {
          this.logger.error(
            `Scanner ${scanner.getName()} failed: ${error.message}`,
          );
      ...
      const scannerInfo =
        scannerInfos.length > 0
          ? scannerInfos[0]
          : { name: 'Multiple Scanners', version: 'latest' };
      ...
      this.logger.log(
        `Raw scan output for ${repoUrl}: ${JSON.stringify(allFindings)}`,
      );
  ...
  private getDefaultMetadata(repoUrl: string): any {
    ...
    const name =
      repoInfo.repoInfo?.repository || this.extractRepoNameFromUrl(repoUrl);
    ...
    try {
      ...
      if (require('path').isAbsolute(filePath)) {
        ...
        if (filePath.startsWith(repoPath)) {
          ...
          const tempDirIndex = pathParts.findIndex((part) =>
            part.startsWith('tmp-'),
          );

          if (tempDirIndex !== -1 && tempDirIndex + 1 < pathParts.length) {
            // Extract path after temp directory
            repositoryRelativePath = pathParts
              .slice(tempDirIndex + 1)
              .join(require('path').sep);
            fullFilePath = require('path').join(
              repoPath,
            ...
            if (require('fs').existsSync(filePath)) {
              ...
              this.logger.warn(
                `Cannot determine repository-relative path for: ${filePath}`,
              );
              return null;
            }
          }
      ...
      if (!require('fs').existsSync(fullFilePath)) {
        this.logger.warn(
          `File not found for context extraction: ${fullFilePath}`,
        );
        return null;
      }
      ...
      this.logger.error(
        `Failed to extract code context for ${filePath}:${line}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  private createStructuredOutput(
    ...
    const totalIssues = Object.values(allFindings).reduce(
    ...
    const summary = {
      ...
      scanners: scannerInfos.map((scanner) => ({
        ...
        version: scanner.version,
                  findingsFound: scanner.findings.length,
    ...
    const details = {
      scanners: scannerInfos.map((scanner) => {
        ...
        return {
          name: scanner.name,
          version: scanner.version,
          totalFindings: scanner.findings.length,
          severityBreakdown: {
